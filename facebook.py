...
root = Tk()
conn = sqlite3.connect('facebook.db')   # the connector class with the database name
c = conn.cursor()   #  helps to execute the query and fetch the records from the database
# c.execute ('''CREATE TABLE user(
#     first_name text,
#     last_name text,
#     age integer,                             #to construct a table
#     address text,
#     city text,
#     zipcode integer,
#     password integer,
...
def submit():
    conn = sqlite3.connect('facebook.db')   #helps to connect to database name.
    c = conn.cursor()     # helps to execute the query and fetch the records from the database.
    c.execute('INSERT INTO user VALUES(:f_name, :l_name,:age, :address,:city,:zipcode,:password,:gender)',{
        'f_name':f_name.get(),
        'l_name':l_name.get(),
        'age':city.get(),
        'address':address.get(),             #run the SQL query and return the result.
        'city':city.get(),
        'zipcode':zipcode.get(),
    ...
    messagebox.showinfo('user','Inserted Successfully')    #provides a template base class.
    conn.commit()
    conn.close()
    f_name.delete(0,END)
    l_name.delete(0,END)
